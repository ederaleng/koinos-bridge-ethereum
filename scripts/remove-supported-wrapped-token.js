...
const WRAPPED_TOKEN_ADDR = '0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6'
...
async function main () {
  ...
  const signatures = await hashAndSign(ActionId.RemoveSupportedWrappedToken, WRAPPED_TOKEN_ADDR, nonce.toString(), bridgeContract.address, nowPlus1Hr)

  console.log('signatures', signatures)

  const tx = await bridgeContract.connect(deployer).removeSupportedWrappedToken(signatures, WRAPPED_TOKEN_ADDR, nowPlus1Hr)
  await tx.wait()

  console.log('Wrapped token support removed', WRAPPED_TOKEN_ADDR)
}
